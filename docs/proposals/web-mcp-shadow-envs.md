...
5. `Date`, typed arrays and buffers get their own tags. `Map` and `Set` have no tag of their own and travel as `{$type: 'object', class: 'Map' | 'Set', preview: {size, entries}}`, the entries rendered under the array limit.
