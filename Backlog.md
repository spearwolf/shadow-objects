...
| `RemoteWorkerEnv` | ✅ **gründlich** — `RemoteWorkerEnv.spec.ts` (45 Fälle über einen Worker-Doppelgänger) deckt Ausfall, Termination, Ablehnung des Ausstehenden und des Nachgereichten, den `destroy()`-Kontrakt samt Abbau während `start()`, das Abtrennen der Transferables ohne Eingriff in den Trail des Aufrufers, die Zuordnung einer Bestätigung über Seriennummer und URL, das Abmelden der Hörer auf allen drei Wegen (Abbau, Ausfall, ein `start()`, dem die Zeit ausgeht), das ausbleibende Bestätigungsfenster, den zurückgehaltenen Ausfall und die zurückgehaltene Load-Meldung gegen einen werfenden Hörer und die Logger-Konfiguration ohne benutzbare Storage sowie mit unbrauchbarem Storage-Wert (unparsbar, Array, `null`, Zahl- und String-Skalar); E2E-Seite `worker-failure` fährt denselben Weg über echtes `postMessage` inklusive Erholung. |
...
15. **API-Aufräumen:** `appendRoute` aufteilen, `onDestroy`-Tripel-Bedeutung dokumentieren oder trennen, `IShadowObjectEnvProxy.isDestroyed`/`error`-Surface ergänzen, Worker-Timeouts konfigurierbar machen.
