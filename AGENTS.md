...
- **Testing:**
    ...
    - `packages/shadow-objects`'s own `test` task holds its built `dist/` output — file list and `dist/package.json` shape — against a recorded expectation (`src/distContract.spec.ts`).
