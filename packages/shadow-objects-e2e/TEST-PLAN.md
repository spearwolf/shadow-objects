...
| MULTI-8 | P2 | Changing `ns` on a live `<shae-ent>` from `alpha` to `beta`: the entity is destroyed in `alpha`, created in `beta`, properties survive the move, and `alpha` receives a sync. |
...
| UPG-7 | P2 | A `<shae-ent>` inside a custom element whose own definition arrives later: parent resolution survives the host upgrade. |
