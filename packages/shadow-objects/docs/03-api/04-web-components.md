...
2.  **Inherited:** If not explicitly set, `shae-*` elements usually default to the global context unless manually configured via JavaScript to inherit. *Note: Currently, declarative inheritance relies on matching `ns` attributes. It is best practice to group elements or explicitly set `ns` if using multiple contexts.*
