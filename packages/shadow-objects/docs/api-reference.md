...
| `removeFromParent(component, parent)` | Detach a component from that parent and make it a root. The promotion follows the detachment: a `parent` whose children list does not name `component` gives nothing up and promotes nothing. A no-op when a later `ViewComponent` has since claimed `component`'s uuid. |
| `moveToRoot(component)` | Make a component a root, taken out of the children list its own `parent` pointer names -- it never ends up under the roots and under that parent at once. A children list a caller has put the component's uuid into by other means, such as `addToChildren()`, is not this method's business and can still name it. A no-op when a later `ViewComponent` has since claimed `component`'s uuid. |
...
| `logger` | `ConsoleLogger` (read-only) | The logger this environment reports through. The slot is a getter without a setter, so `env.logger = …` throws a `TypeError` in strict mode and does nothing outside it. |
...
Over a worker the same call costs one round trip. The request travels to the worker as an `Inspect` message, the Kernel builds the snapshot there -- `kernel.thread` reads `'worker'` -- and the answer comes back as `Inspected`, matched to its request by a serial of its own. The request runs through the same queue as the change trails, so the snapshot reflects every trail posted before the call and none posted after it; that is why the `syncWait()` above is enough on both kinds of environment. An answer that stays out past `inspectTimeout` is reported under `error` as a `WorkerTimeoutError` naming `'inspected'`, and a worker that fails while it answers is reported the same way, as a `WorkerFailedError` -- the environment survives the loss of its proxy and can be given a new one, see [When the Worker Dies](./guides.md#when-the-worker-dies). Only a `ShadowEnv` destroyed before or while the proxy answers rejects the call, as the first bullet says. See [`RemoteWorkerEnv`](#remoteworkerenv).
...
| `cause` | `unknown` | What the entry threw. The error object itself in a local environment; across a worker boundary a `WorkerReportedError` carrying its wording and its name, without the class of the original error and without the fields that class added. |
...
| `onParentChanged(newParent, oldParent)` | Called when the node the element hangs on and the one it leaves are two different nodes — an atomic `moveBefore()` that reorders it among its siblings is not reported. Re-resolves the entity ancestor; an extension point for subclasses, which have to call `super`. A throw costs only this element and is reported through `console.error`. |
...
Only a local environment hands the object itself to the view side. A Worker puts the wording and the
name of the refusal on the wire, so the `cause` of the `ChangeTrailRefusedError` that arrives there
is a `WorkerReportedError` with `name === 'EntityUuidInUseError'` -- and it carries no `uuid` field,
because those two are the only fields that cross the boundary. The same limit the
`ChangeTrailRefusedError` section names.
