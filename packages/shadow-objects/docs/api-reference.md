...
One kind of listener stands outside this: a callback registered through `on(onDestroy, …)` or `once(onDestroy, …)` on the creation API instead of this method. It carries no priority of its own, which puts it ahead of the creation scopes and of the Entity's own destruction, and a throw there costs both — every Shadow Object of that Entity keeps the cleanups it registered through `onDestroy(callback)`, and the Entity keeps its properties, contexts and subscriptions. It is only ever reached on the first path, because a destroyed Entity is the only occasion on which the Entity-wide notification is sent. What such a throw does not touch is the Kernel's own bookkeeping: the Entity is out of the Kernel when `destroyEntity()` returns, whatever ran or failed along the way, and the call hands nothing on to its caller.
...
| `destroyEntity` | `(uuid: string)` | Destroys the Entity, its Shadow Objects and its children. Each Shadow Object is told on its own, so a teardown that throws costs no sibling its own; the throw is reported through the `ConsoleLogger` rather than handed to the caller, and the Entity is out of the Kernel when the call returns either way. |
