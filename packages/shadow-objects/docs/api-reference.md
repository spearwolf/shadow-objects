...
A listener sitting on the Entity itself is held to the same contract, wherever in the delivery it stands — registered through `on(onDestroy, …)` / `once(onDestroy, …)` on the creation API, which carries no priority of its own, or put on the Entity from the outside ahead of that, `Priority.Max` included, where a child with `autoDestructionOnParentRemoval` registers: a throw from it costs no Shadow Object of that Entity the cleanups it registered through `onDestroy(callback)`, and costs the Entity neither its properties nor its contexts nor its subscriptions. The Kernel runs the creation scopes and the Entity's own release itself once the Entity-wide notification is through, however that notification ended. Such a listener is only ever reached on the first path, because a destroyed Entity is the only occasion on which that notification is sent.
...
| `destroyEntity` | `(uuid: string)` | Destroys the Entity, its Shadow Objects and its children. Each Shadow Object is told on its own, so a teardown that throws costs no sibling its own; the throw is reported through the `ConsoleLogger` rather than handed to the caller, and the Entity is out of the Kernel when the call returns either way. A listener on the Entity's own destruction notification is held to the same contract: the creation scopes tear down and the Entity releases its properties, contexts and subscriptions whether or not that notification reached them — see [`onDestroy(callback)`](#ondestroycallback). |
