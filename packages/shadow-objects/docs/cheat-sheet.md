...
| `shae-get-entity` | `uuid, namespace?` | one Entity, its ancestors, the View component next to it, failed environments under `errors` |
...
Read-only, every value untrusted by declaration, a refusal is `isError: true`.
